...
#         rooms = Room.objects.all()
#     else:
#         # A more robust query using a direct reverse lookup to prevent potential errors.
#         rooms = Room.objects.filter(
#             invigilatorassignment__invigilator=request.user
#         ).distinct()
...
def dashboard(request):
    if request.user.is_staff:
        ...
        # A more explicit two-step query to prevent production database errors.
        assigned_room_ids = InvigilatorAssignment.objects.filter(
            invigilator=request.user
        ).values_list('room_id', flat=True)
        rooms = Room.objects.filter(id__in=assigned_room_ids).distinct()
